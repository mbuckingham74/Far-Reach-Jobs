...
def check_robots_blocked(source: ScrapeSource) -> tuple[bool, str | None, str | None, str | None]:
    ...
    to avoid accidentally crawling sites whose policy we couldn't read.
...
def _run_adp_scraper(
    ...
    Args:
        listing_urls: List of ADP WorkforceNow listing URLs to scrape
    """
    ...
    for listing_url in listing_urls:
        try:
            ...
            all_errors.append(f"ADP scraper execution failed for {listing_url}: {e}")
            logger.exception(f"ADP scraper failed for {source.name} URL: {listing_url}")
